...
var Router = function(adapter) {
    ...
    this.adapter.on('message', function(topic, message) {
    ...
    this.adapter.on('connect', function() {
        log('connect', self.adapter.connected);
        ...
        self.endpoints.forEach(function(endpoint) {
            ...
            self.adapter.subscribe(endpoint.topic, endpoint.opts);
        });
    });

    this.adapter.on('close', function(err) {
        log('close', self.adapter.connected, err);
    });
    ...
    this.subscribe = function(topic, opts, handler) {
        ...
        this.adapter.subscribe(safeTopic, opts);
